            ...
            tabPanel(title = 'Methods', 
                     ...
                     checkboxInput(
                       ...
                       fluidRow(
                         column(
                           width = 6, 
                           DT::DTOutput("ogRegSummary")
                         ), 
                         column(
                           width = 6, 
                           DT::DTOutput("regSummary")
                         )
                       ), 
                       br(), 
                       ...
                       fluidRow(
                         column(
                           width = 6, 
                           DT::DTOutput(outputId = "originalDT")
                         ), 
                         column(
                           width = 6, 
                           DT::DTOutput("impValues")
                         )
                       ), 
                     br()
            )
...
server <- function(input, output, session) {
  ...
  output$countNas <- renderDT({
    if (input$inputLevel2 == "Diabetes"){
      ...
      Number_of_NAs <- 
        df1$sapply.diabetes..function.x..sum.is.na.x...[1:8] 
      ...
      Number_of_NAs <- 
        df2$sapply.iris20..function.x..sum.is.na.x...[1:4]
      as.data.frame(rbind(Variables, Number_of_NAs))
    }
  }, 
  style = "bootstrap4", 
      ...
      output$diabetesPlot = renderPlot({
        {
          ...
          {
            data.lm = lm(diabetes[, input$dataIndeVar]~ diabetes$Age)
            ggplot(diabetes, aes(Age, diabetes[, input$dataIndeVar])) +
              geom_point() +
              labs(x='Age',y=input$dataIndeVar) +
          ...
          {
            diabetes2 = diabetes
            round(mean(diabetes2[, input$dataIndeVar], 
                       na.rm = TRUE), digits = 0) -> avg
            replace_na(diabetes2[, input$dataIndeVar], 
                       replace = avg) -> diabetes2[, input$dataIndeVar]
            data.lm2 = lm(diabetes2[, input$dataIndeVar]~ diabetes2$Age)
            ggplot(diabetes2, aes(Age, diabetes2[, input$dataIndeVar])) +
              geom_point() + labs(x='Age',y=input$dataIndeVar) +
              theme_bw(base_size = 20) +
          ...
          {
            diabetes3 = diabetes
            replace_na(diabetes3[, input$dataIndeVar], 
                       replace = 0) -> diabetes3[, input$dataIndeVar]
            data.lm3 = lm(diabetes3[, input$dataIndeVar]~ diabetes3$Age)
            ggplot(diabetes3, aes(Age, diabetes3[, input$dataIndeVar])) +
              geom_point() + labs(x='Age',y=input$dataIndeVar) +
              theme_bw(base_size = 20) +
  ...
  ## Level 3 ----
  # Full Dataset
  output$ogRegSummary <- DT::renderDT({
    if (input$inputLevel3 == "Iris") {
      reg0 <- lm(Sepal.Length ~ Sepal.Width + Petal.Length + Species, data = iris)
      Variables <- c('(intercept)', 'Sepal.Width', 'Petal.Length', 'Speciesversicolor', 'Speciesvirginica')
    } 
    else if (input$inputLevel3 == "Diabetes") {
      reg0 <- lm(Age ~ Pregnancies + Glucose + BloodPressure + SkinThickness + Insulin + BMI + DiabetesPedigreeFunction, data = diabetes)
      Variables <- c('(intercept)', 'Pregnancies', 'Glucose', 'BloodPressure', 'SkinThickness', 'Insulin', 'BMI', 'DiabetesPedigreeFunction')
    }
    sum_coef <- summary(reg0)[4]
    Estimates <- format(sum_coef$coefficients[1:5], digits = 2)
    Std.Error <- format(sum_coef$coefficients[6:10], digits = 2)
    t_value <- format(sum_coef$coefficients[11:15], digits = 2)
    p_value <- format(sum_coef$coefficients[16:20], digits = 2)
  ...
  #Generate sample DT from Iris or Diabetes
  output$originalDT <- DT::renderDT({
    if(input$inputLevel3 == "Iris"){
    iris[random1,]}
    else if(input$inputLevel3 == "Diabetes"){
      diabetes[random3,]
    ...
    #Iris dataset
      if (input$imp_methods == 'Complete Case Analysis') {
        output$regSummary <- DT::renderDT({
          ...
          Estimates <- format(sum_coef3$coefficients[1:5], digits = 2)
          Std.Error <- format(sum_coef3$coefficients[6:10], digits = 2)
          t_value <- format(sum_coef3$coefficients[11:15], digits = 2)
          p_value <- format(sum_coef3$coefficients[16:20], digits = 2)
          as.data.frame(cbind(Variables, Estimates, Std.Error, t_value, p_value))
        }, 
        style = "bootstrap4",
        rownames = TRUE, 
        ...
        output$regSummary <- DT::renderDT({
          ...
          Estimates <- format(sum_coef4$coefficients[1:5], digits = 2)
          Std.Error <- format(sum_coef4$coefficients[6:10], digits = 2)
          t_value <- format(sum_coef4$coefficients[11:15], digits = 2)
          p_value <- format(sum_coef4$coefficients[16:20], digits = 2)
          as.data.frame(cbind(Variables, Estimates, Std.Error, t_value, p_value))
        }, 
        style = "bootstrap4", 
        rownames = TRUE, 
        ...
        output$regSummary <- DT::renderDT({
          ...
          Estimates <- format(sum_coef5$coefficients[1:5], digits = 2)
          Std.Error <- format(sum_coef5$coefficients[6:10], digits = 2)
          t_value <- format(sum_coef5$coefficients[11:15], digits = 2)
          p_value <- format(sum_coef5$coefficients[16:20], digits = 2)
          as.data.frame(cbind(Variables, Estimates, Std.Error, t_value, p_value))
        },
        style = "bootstrap4", 
        rownames = TRUE, 
        ...
        output$regSummary <- DT::renderDT({
         ...
         sum_coef2 <- summary(pooled)
          Estimates <- format(sum_coef2$coefficients[1:5], digits = 2)
          Std.Error <- format(sum_coef2$coefficients[6:10], digits = 2)
          t_value <- format(sum_coef2$coefficients[11:15], digits = 2)
          p_value <- format(sum_coef2$coefficients[16:20], digits = 2)
          as.data.frame(cbind(Variables, Estimates, Std.Error, t_value, p_value))
        },
        style = "bootstrap4", 
        rownames = TRUE, 
